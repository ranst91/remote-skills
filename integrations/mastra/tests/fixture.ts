...
export function fixture(
  ...
  options: { token?: string; extraSkill?: boolean } = {},
) {
  const token = options.token ?? TOKEN;
  const archive = (version: string, name = "greeting") =>
    encodeZip([
      {
        path: "SKILL.md",
        bytes: bytes(
  ...
  const otherArtifact = archive("1.0.0", "weather");
  ...
  const dependencies: RemoteSkillsDependencies = {
    ...
    transport: async (request) => {
                  ...
                  ? [
                      {
                        ...
                        description: "Weather",
                        type: "archive",
                        url: "artifacts/weather.zip",
                        digest: digest(otherArtifact),
                      },
                    ]
                  : []),
                {
      ...
      if (path.endsWith("weather.zip"))
        return { status: 200, headers: { "content-type": "application/zip" }, body: otherArtifact };
