...
try {
  ...
  writeLockedIntegrationProject(root, work, "integrations/tanstack-ai");
  ...
  copyFileSync(
    join(root, "integrations/tanstack-ai/tests/installed-consumer.ts"),
    join(work, "check.ts"),
  );
  const result = spawnSync(
    ...
    [
      "check.ts",
      stringField(
        manifestObject(readFileSync(join(root, "integrations/tanstack-ai/package.json"), "utf8")),
        "version",
      ),
    ],
    {
  ...
  if (result.status !== 0)
    throw new Error(`Installed TanStack AI check failed: ${result.stdout}\n${result.stderr}`);
