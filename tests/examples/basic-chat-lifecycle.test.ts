...
import { syncPython } from "../../scripts/examples/sync-python.ts";
...
test("missing credentials fail without disclosing their value", () => {
  ...
  assert.throws(
    () => validateEnvironment("typescript", { OPENAI_MODEL: sentinel }),
    (error: unknown) => {
...
test("Python setup reports unavailable uv without echoing the attempted executable", () => {
  const sentinel = "missing-uv-secret-sentinel";
  assert.throws(
    () =>
      syncPython({
        cwd: mkdtempSync(join(tmpdir(), "remote-skills-python-sync-")),
        executable: sentinel,
        required: true,
      }),
    (error: unknown) => {
      assert(error instanceof Error);
      assert.match(error.message, /uv.*required/u);
      assert.equal(error.message.includes(sentinel), false);
      return true;
    },
  );
