...
export type Example = "typescript" | "python";
export const DUMMY_KEY = "not-a-real-key-basic-chat-private-sentinel";
...
async function ports() {
  const reserved = await Promise.all([reservePort(), reservePort(), reservePort()]);
  const [app, agent, skills] = reserved.map((entry) => entry.port);
  assert.ok(app && agent && skills);
  await Promise.all(reserved.map((entry) => entry.close()));
  return { app, agent, skills };
}
...
export async function disposableExample(example: Example) {
  const root = await mkdtemp(resolve(tmpdir(), "remote-skills-basic-chat-"));
  ...
  try {
    ...
    if (example === "typescript") env.REMOTE_SKILLS_UV = resolve(root, "unavailable-uv");
    ...
    try {
      assert.equal(
        ...
        0,
        `Install from basic-${example} succeeds.\n${install.output()}`,
      );
    } finally {
    ...
    if (example === "python")
      assert.ok(
        existsSync(resolve(exampleRoot, "agent/.venv")),
        "pnpm install creates the Python environment.",
      );
...
export async function startChat(exampleRoot: string, modelUrl: string) {
  ...
  const env = {
    ...
    AGENT_PORT: String(selected.agent),
  ...
  try {
    await until(
      ...
      "all three example services become ready",
      120_000,
    );
    ...
    assert.ok(
      processes.some((entry) => entry.command.includes("vite/bin/vite.js")),
      "The app is running.",
    );
    assert.ok(
      processes.some(
        (entry) =>
          entry.command.includes("agent/src/server.ts") ||
          entry.command.includes("uvicorn server:app"),
      ),
      "The real language backend is running.",
    );
