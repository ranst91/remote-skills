...
Successful commands print a short summary: `validate` reports skill counts, `build` also reports its output directory, and `verify` reports the origin and verified skill/artifact counts. Warnings and errors go to stderr; failures return a nonzero exit code.
...
npx @remote-skills/cli validate --strict
```
...
```bash
npx @remote-skills/cli build --format tar.gz --out-dir dist
```
...
```bash
npx @remote-skills/cli verify https://skills.example.com --timeout-ms 30000 --retries 2
```
