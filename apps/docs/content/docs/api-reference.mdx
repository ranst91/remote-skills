...
Reference for `@remote-skills/client`, Python's `remote_skills`, the [Vercel AI SDK integration](#vercel-ai-sdk-integration), and the [LangChain and DeepAgents integration](#langchain-integration). Method tables show awaited return values: TypeScript methods return promises, and Python async methods are awaited. For a runnable integration, see [Consume skills](/docs/consume).
