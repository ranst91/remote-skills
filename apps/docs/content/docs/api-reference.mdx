...
Reference for `@remote-skills/client`, Python's `remote_skills`, and the [Vercel AI SDK integration](#vercel-ai-sdk-integration). Method tables show awaited return values: TypeScript methods return promises, and Python async methods are awaited. For a runnable integration, see [Consume skills](/docs/consume).
