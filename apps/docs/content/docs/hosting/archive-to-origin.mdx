...
Use the scoped package name `@remote-skills/cli` with `npx`. The unscoped `remote-skills` name belongs to a different npm package. With the [CLI installed](/docs/quickstart#serve-it), this runs your installed version; otherwise npm offers to install it temporarily.
...
A successful build reports the number of skills and the output directory. Check `dist/.well-known/agent-skills/index.json` for your skills and the generated `artifacts/` directory for their files.
