...
const generatedUnicodeSource = path.join(
  packageDirectory,
  "src",
  "authoring",
  "unicode-case-fold-v15.mjs",
);
await cp(
  generatedUnicodeSource,
  path.join(outputDirectory, "authoring", "unicode-case-fold-v15.mjs"),
);

const vendorSource = path.join(packageDirectory, "src", "build", "vendor");
const vendorOutput = path.join(outputDirectory, "build", "vendor");
await mkdir(vendorOutput, { recursive: true });
await Promise.all(
  ["pako-deflate.mjs", "pako.LICENSE.txt"].map((name) =>
    cp(path.join(vendorSource, name), path.join(vendorOutput, name)),
  ),
);
