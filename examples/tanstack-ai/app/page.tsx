...
import { WELCOME_PROMPT } from "./prompt.ts";
  ...
  return (
    <main className="shell">
      <header className="intro">
        <p className="eyebrow">Remote Skills · TanStack AI</p>
        ...
        <div id="conversation" className="conversation" role="log" aria-live="polite">
          {messages.length === 0 && (
            <p className="empty-state">
              Welcome a teammate using your team's prescribed greeting style.
            </p>
          )}
            ...
            <article key={message.id} className={`message ${message.role}`}>
              ...
              {message.parts.map((part, index) => {
                ...
                if (
                  ...
                  part.type === "tool-list_skill_resources"
                )
                  return (
                    <details key={part.toolCallId}>
                      <summary>
                        {part.type === "tool-load_skill" ? "Skill loading" : "Skill file read"} ·{" "}
                        {part.state}
                      </summary>
